...
public class TC_ID_009_EMI_Test extends Driver_Setup{
	...
	public boolean TC_TestCaseName() throws Throwable 
	{
		try {						
			...
			if(Excel_Handling.Get_Data(ModuleName, "ProceedWithNavigation").trim().equalsIgnoreCase("Y"))
			{
				...
				boolean flag = 	MA_panel.ModuleName(Excel_Handling.Get_Data(ModuleName, "Fields").trim(), "//*[text()='"+Excel_Handling.Get_Data(ModuleName, "Fields").trim()+"']");		
