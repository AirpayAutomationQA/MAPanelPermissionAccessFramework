...
public class TC_ID_002_Searchinner extends Driver_Setup{
		...
		try {						
			...
			{						
			    ...
			    {			    	
					...
					if(MA_panel.Home_LHS_Dashboard(AirPay_Payment_MA_Panel_PageObject.SearchInnerLink)==true)
					{
						Extent_Reporting.Log_Pass("Search Link is exist", "Passed");
						Extent_Reporting.Log_report_img("Search Link", "Passed", webDriver);
						test.NavigationWithTodaysDate("Pass");						
					}else{
						Extent_Reporting.Log_Fail("SearchLink does not exist", "Failed", webDriver);
			...
			{				
			   if(Excel_Handling.Get_Data(ModuleName, "PermissionAccess").trim().equalsIgnoreCase("Y"))
				{			    	
					...
					{
						if(driver.findElements(By.xpath(AirPay_Payment_MA_Panel_PageObject.SearchInnerLink)).size()>0)
						{
							Extent_Reporting.Log_Pass("Search Link is exist", "Passed");
							Extent_Reporting.Log_report_img("Search Link", "Passed", webDriver);
