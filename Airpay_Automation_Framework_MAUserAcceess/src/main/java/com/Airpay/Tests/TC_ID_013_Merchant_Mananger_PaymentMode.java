...
public class TC_ID_013_Merchant_Mananger_PaymentMode extends Driver_Setup{
		...
		try {						
			...
			{				
					...
					if(Excel_Handling.Get_Data(ModuleName, "PermissionAccess").trim().equalsIgnoreCase("N"))
					 {
						if(driver.findElements(By.xpath("//*[text()='Merchant Manager']")).size()==0)
						{				
						 ...
						 if(MA_panel.Home_LHS_Dashboard(AirPay_Payment_MA_Panel_PageObject.PaymentModeMenu)==true)
							{
								Extent_Reporting.Log_Pass("Home Menu Is exist", "Passed");
								Extent_Reporting.Log_report_img("Home Menu", "Passed", driver);
